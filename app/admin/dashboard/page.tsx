...
export default async function AdminDashboard() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-deep-green mb-8">
        ...
        {stats.map((stat) => {
          ...
          return (
            <Card key={stat.title} className="border-gold-500/30">
              ...
              <CardContent>
                <div className="text-3xl font-bold text-deep-green">{stat.value}</div>
              </CardContent>
            </Card>
          );
        })}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Link href="/tree" className="group">
          <Card className="bg-deep-green text-white border-gold-500/50 hover:bg-deep-green/90 transition-all shadow-xl h-full">
            <CardContent className="p-8 flex flex-col items-center text-center">
              <TreePine className="w-16 h-16 text-gold-500 mb-4 group-hover:scale-110 transition-transform" />
        ...
        <Link href="/admin/people" className="group">
          <Card className="bg-gold-500 text-deep-green border-gold-500/50 hover:bg-gold-600 transition-all shadow-xl h-full">
            <CardContent className="p-8 flex flex-col items-center text-center">
              <Users className="w-16 h-16 text-deep-green mb-4 group-hover:scale-110 transition-transform" />
              <h2 className="text-2xl font-bold mb-2">إدارة الأشخاص</h2>
              <p className="text-deep-green/80">أضف، عدّل، واحذف الأشخاص في قاعدة البيانات</p>
            </CardContent>
          </Card>
        </Link>
      </div>
